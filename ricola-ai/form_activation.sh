...
form_activation_guide="FORM-ACTIVATION-STEPS-2.rtf"
if [ -e "./template/$form_activation_guide" ]
then
	cp ./template/$form_activation_guide ./$folderA/$tempFolder/$folderB/$myfolderA/$client_web/$form_activation_guide
	start wordpad.exe ./$folderA/$tempFolder/$folderB/$myfolderA/$client_web/$form_activation_guide
...
echo "FORM NAME"
echo "--------------------------"
read -p "Enter form name:" form_name 
while [[ "${form_name}" == "" ]] ; do
   read -p "Enter form name:" form_name   
done
cscript //nologo "textReplace.vbs" "form_name" "$form_name" ./$folderA/$tempFolder/$folderB/$myfolderA/$client_web/$file_name

#form_name 
echo "--------------------------"
echo "FORM HOME LOCATION"
echo "--------------------------"
read -p "Enter form location (ex: bottom):" form_location 
while [[ "${form_location}" == "" ]] ; do
   read -p "Enter form location (ex: bottom):" form_location  
done
cscript //nologo "textReplace.vbs" "form_location" "$form_location" ./$folderA/$tempFolder/$folderB/$myfolderA/$client_web/$file_name
...
echo "FORM LINK"
echo "--------------------------"
read -p "Enter form link:" form_link 
while [[ "${form_name}" == "" ]] ; do
   read -p "Enter form link:" form_link   
done
cscript //nologo "textReplace.vbs" "form_link" "$form_link" ./$folderA/$tempFolder/$folderB/$myfolderA/$client_web/$file_name
