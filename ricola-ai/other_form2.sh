...
read -p "Enter New FORM TITLE:" tempFolder  
	...
	echo "$tempFolder" > ./tempFolder2.txt 
	cscript //nologo "textReplace.vbs" " " "-" ./tempFolder2.txt 

	#_scrn_new="${scrn_new}" | sed -E 's/ /-/'
	#scrn_newfile=$_scrn_new".vbs"
				
	if [ -e "./tempFolder2.txt" ]
	then
		tempFolder="$(<./tempFolder2.txt)"
		echo $tempFolder										
	else
...
#cp ./template/$file_name ./$folderA/$tempFolder/$folderB/$myfolderA/$client_web/$file_name
#start wordpad.exe ./$folderA/$tempFolder/$folderB/$myfolderA/$client_web/$file_name
if [ -e "./open_form.vbs" ]
then
	echo "File existing"
